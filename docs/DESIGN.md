...
| ReadyToRun follows trimming: on for the CLI, off for the tray app | Untrimmed, framework assemblies keep the precompiled native code they ship with, and recompiling it only enlarges the payload — measured 68.6 MB → 72.7 MB with cold start 1.9 s → 4.7 s for the tray app. Trimming discards that code, which is why the CLI has to put it back. |
