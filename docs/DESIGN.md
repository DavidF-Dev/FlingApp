...
| CLI is trimmed; tray app is not | Trimming is worth 33.7 MB → 10.5 MB and roughly halves cold start, which matters because the CLI's main callers are Greenshot and Explorer "Send to". WPF is not trim-compatible, so the tray app accepts a larger binary. The two front-ends do not share publish settings. |
