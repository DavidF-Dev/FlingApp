...
- **The startup entry repairs itself when it goes stale.** Detecting a mismatch and reporting the checkbox as off was honest but unhelpful: the entry still launched nothing. On startup, an entry that differs from what this build would write is rewritten. Deliberately narrow — an entry is never created, a copy that still exists is never displaced, and a refusal recorded in Task Manager survives, since correcting an entry is not consent to start again.

  Originally this covered only a *moved* executable, which turned out to be half the problem. An entry written before `--minimized` existed still named a valid path, so nothing looked wrong — and the app dutifully opened a window at every sign-in. Arguments go stale between versions as readily as paths do, and the repair now covers both.
...
- [x] Tray tooltip stays a plain "Fling" — a device count there is noise, not information.
...
- **The tray tooltip is just "Fling".** A paired device count was built and then removed: the hover text is glanced at, not read, and the count is already on screen in the Device manager.
